...
export const motionSchema = z.discriminatedUnion('kind', [
  ...
  z.object({ ...commonMotion, kind: z.literal('frame_animation'), frames: z.array(relativePath).min(1).max(240), fps: finite.min(1).max(60).optional(), frameDurationsMs: z.array(duration).max(240).optional(), canvas: z.object({ width: finite.int().min(1).max(4096), height: finite.int().min(1).max(4096) }).strict(), anchor: z.object({ x: finite.min(0).max(1), y: finite.min(0).max(1) }).strict() }).strict()
]);
export type Motion = z.infer<typeof motionSchema>;
export type ActionNode = { type: 'sequence'; children: ActionNode[] } | { type: 'parallel'; children: ActionNode[] } | { type: 'motion'; motionId: string; repeat: number } | { type: 'expression'; expressionId: string; durationMs: number } | { type: 'move_by'; dxDip: number; dyDip: number; durationMs: number; easing: Easing } | { type: 'effect'; effectId: string; durationMs: number } | { type: 'bubble'; text: string; durationMs: number } | { type: 'wait'; durationMs: number };
export const nodeSchema: z.ZodType<ActionNode> = z.lazy(() => z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('effect'), effectId: z.enum(effects), durationMs: duration }).strict(),
...
export function makeCatalog(registry: Registry): Catalog {
  ...
  for (const p of registry.packs) {
    for (const m of p.motions) if (m.kind === 'frame_animation') for (const frame of m.frames) if (!assets.has(`${p.manifest.id}:${frame}`)) throw new Error(`缺少图片：${frame}`);
